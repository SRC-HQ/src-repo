...
var racerSettings = [
  {
    ...
    odds: [2.38, 1.25, 1.1, 2.38, 2.38, 2.38, 2.38, 2.38, 2.38],
    percent: 80,
  },
  {
    name: 'Doctor',
    ...
    odds: [2.75, 1.29, 1.11, 2.75, 2.75, 2.75, 2.75, 2.75, 2.75],
    percent: 80,
  },
  {
    name: 'Astronaut',
    ...
    odds: [4.5, 3.0, 2.25, 4.5, 4.5, 4.5, 4.5, 4.5, 4.5],
    percent: 80,
  },
  {
    name: 'Bartender',
    ...
    odds: [6.0, 3.4, 1.5, 6.0, 6.0, 6.0, 6.0, 6.0, 6.0],
    percent: 70,
  },
  {
    name: 'Teacher',
    ...
    odds: [7.5, 3.76, 1.55, 7.5, 7.5, 7.5, 7.5, 7.5, 7.5],
    percent: 70,
  },
  {
    name: 'Chef',
    ...
    odds: [8.5, 6.0, 3.25, 8.5, 8.5, 8.5, 8.5, 8.5, 8.5],
    percent: 50,
  },
  {
    name: 'War Admiral',
    ...
    odds: [9.5, 7.5, 3.12, 9.5, 9.5, 9.5, 9.5, 9.5, 9.5],
    percent: 50,
  },
  {
    name: 'Farmer',
    ...
    odds: [9.75, 5.2, 2.48, 9.75, 9.75, 9.75, 9.75, 9.75, 9.75],
    percent: 30,
  },
  {
    name: 'Engineer',
    ...
    odds: [11.65, 7.89, 3.44, 11.65, 11.65, 11.65, 11.65, 11.65, 11.65],
    percent: 30,
  },
  {
    name: 'Police Officer',
    ...
    odds: [17.25, 8.75, 6.3, 17.25, 17.25, 17.25, 17.25, 17.25, 17.25],
    percent: 30,
  },
];
var betSettings = [
  ...
  switch (page) {
    ...
    case 'result':
      ...
      TweenMax.to(tweenData, 0.5, {
        win: playerData.totalWin,
        overwrite: true,
        onUpdate: function () {
          resultDescTxt.text = textStrings.resultDesc.replace(
            '[NUMBER]',
            addCommas(Math.round(tweenData.win)),
          );
        },
      });
...
function resizeGameLayout() {
  ...
  if (viewport.isLandscape) {
    ...
    scoreContainer.y = (canvasH / 100) * 81;
    ...
    scoreContainer.x = canvasW / 2;
    scoreContainer.y = (canvasH / 100) * 80;
...
function toggleBetType(con) {
  if (con) {
    ...
    gameData.betTypeIndex =
      gameData.betTypeIndex > betSettings.length - 1 ? 0 : gameData.betTypeIndex;
  } else {
    gameData.betTypeIndex--;
    gameData.betTypeIndex =
      gameData.betTypeIndex < 0 ? betSettings.length - 1 : gameData.betTypeIndex;
  }
...
function toggleBetAmount(con) {
  if (con) {
    ...
    gameData.betAmountIndex =
      gameData.betAmountIndex > betSettings[gameData.betTypeIndex].bets.length - 1
        ? 0
        : gameData.betAmountIndex;
  } else {
    gameData.betAmountIndex--;
    gameData.betAmountIndex =
      gameData.betAmountIndex < 0
        ? betSettings[gameData.betTypeIndex].bets.length - 1
        : gameData.betAmountIndex;
  }
...
function updateBetOptions() {
  ...
  betAmountTxt.text =
    textStrings.currency +
    addCommas(betSettings[gameData.betTypeIndex].bets[gameData.betAmountIndex]);
  ...
  for (var n = 0; n < columnPos.length; n++) {
    if (n == 0) {
      ...
      if (betType == 'win') {
        ...
        columnDisplay.push({
          type: 'CHECKBOX',
          name: textStrings.position[0] + textStrings.or + textStrings.position[1],
          x: columnPos[1],
          align: 'left',
        });
        ...
        columnDisplay.push({
          type: 'CHECKBOX',
          name:
            textStrings.position[0] +
            textStrings.or +
            textStrings.position[1] +
            textStrings.or +
            textStrings.position[2],
          x: columnPos[1],
          align: 'left',
        });
  ...
  for (var n = 0; n < racerSettings.length; n++) {
    for (var c = 0; c < columnDisplay.length; c++) {
      if (columnDisplay[c].type == 'POS') {
        ...
        $.racer[n].scaleX = $.racer[n].scaleY = 0.5;
  ...
  for (var n = 0; n < racerSettings.length; n++) {
    for (var c = 0; c < columnDisplay.length; c++) {
      if (columnDisplay[c].type == 'POS') {
        ...
        $.racer[n + '_p'].scaleX = $.racer[n + '_p'].scaleY = 0.5;
...
function toggleBetTable(con) {
  ...
  TweenMax.to(betContentContainerP, 0.2, { x: newX, overwrite: true });
}
...
function toggleSummaryTable(con) {
  ...
  TweenMax.to(betSummaryContentContainerP, 0.2, { x: newX, overwrite: true });
}
...
function toggleRaceSummaryTable(con) {
  ...
  TweenMax.to(raceSummaryContentContainerP, 0.2, { x: newX, overwrite: true });
}
...
function toggleRaceTable(con) {
  ...
  TweenMax.to(raceResultContentContainerP, 0.2, { x: newX, overwrite: true });
}
...
function updateTotalBet() {
  ...
  gameData.totalBet =
    betSettings[gameData.betTypeIndex].bets[gameData.betAmountIndex] * combination;
...
function tryPlaceBet(con) {
  ...
  if (con) {
    if (playerData.credit >= gameData.totalBet) {
      playSound('soundBet');
      playerData.bets.push({
        type: gameData.betTypeIndex,
        racer: racerNumbers,
      ...
      animateCredit(creditRedTxt, 0.5);
    }
  }
}
...
function updateRaceTable() {
  ...
  for (var n = 0; n < gameData.revealResults.length; n++) {
    ...
    TweenMax.from(newPos, 0.5, { delay: delayCount, x: newPos.x + 30, alpha: 0, overwrite: true });
    TweenMax.from(newIcon, 0.5, {
      delay: delayCount,
      x: newIcon.x + 30,
      alpha: 0,
      overwrite: true,
    });
    TweenMax.from(newName, 0.5, {
      delay: delayCount,
      x: newName.x + 30,
      alpha: 0,
      overwrite: true,
...
function updateRaceSummaryTable() {
  ...
  winnerGlow.graphics
    .beginRadialGradientFill(
      ['rgba(255,238,88,0.6)', 'rgba(255,238,88,0)'],
      [0, 1],
      0,
      0,
      0,
  ...
  var spacingWN = 12;
  var iconW =
    winnerIcon.image && winnerIcon.image.naturalWidth ? winnerIcon.image.naturalWidth : 40;
  ...
  var spacingWNP = 12;
  var iconWP =
    winnerIconP.image && winnerIconP.image.naturalWidth ? winnerIconP.image.naturalWidth : 40;
...
function animateWinHighlight(itemHighlight, delay, winTxt, amount, update) {
  TweenMax.to(itemHighlight, 0.3, {
    ...
    onComplete: function () {
      ...
      animateBlink(itemHighlight, 0.5);
      ...
      winTxt.amount = 0;
      TweenMax.to(winTxt, gameSettings.winSpeed, {
        amount: amount,
        overwrite: true,
        onUpdate: function () {
...
function updateSummary(amount) {
  ...
  TweenMax.to(gameData.summaryCredits, gameSettings.winSpeed, {
    amount: newCredit,
    overwrite: true,
    onUpdate: function () {
      gameData.summaryCredits.text =
        ...
        ' : ' +
        textStrings.currency +
        addCommas(Math.round(gameData.summaryCredits.amount));
    },
  });
  TweenMax.to(gameData.summaryCreditsP, gameSettings.winSpeed, {
    amount: newCredit,
    overwrite: true,
    onUpdate: function () {
      gameData.summaryCreditsP.text =
        ...
        ' : ' +
        textStrings.currency +
        addCommas(Math.round(gameData.summaryCreditsP.amount));
    },
  });

  var newWin = gameData.summaryWin.amount + amount;
  TweenMax.to(gameData.summaryWin, gameSettings.winSpeed, {
    amount: newWin,
    overwrite: true,
    onUpdate: function () {
      gameData.summaryWin.text =
        textStrings.totalWin +
        ' : ' +
        textStrings.currency +
        addCommas(Math.round(gameData.summaryWin.amount));
    },
  });
  TweenMax.to(gameData.summaryWinP, gameSettings.winSpeed, {
    amount: newWin,
    overwrite: true,
    onUpdate: function () {
      gameData.summaryWinP.text =
        ...
        ' : ' +
        textStrings.currency +
        addCommas(Math.round(gameData.summaryWinP.amount));
    },
  });
}
...
function checkWinAmount(index) {
  ...
  if (betType == 'win') {
    if (gameData.revealResults[0] == playerData.bets[index].racer[0]) {
      winData.win = true;
      winData.amount +=
        racerSettings[playerData.bets[index].racer[0]].odds[betIndex] *
        playerData.bets[index].totalBet;
    }
  } else if (betType == 'place') {
    if (
      gameData.revealResults[0] == playerData.bets[index].racer[0] ||
      gameData.revealResults[1] == playerData.bets[index].racer[0]
    ) {
      winData.win = true;
      winData.amount +=
        racerSettings[playerData.bets[index].racer[0]].odds[betIndex] *
    ...
    if (
      gameData.revealResults[0] == playerData.bets[index].racer[0] ||
      gameData.revealResults[1] == playerData.bets[index].racer[0] ||
      gameData.revealResults[2] == playerData.bets[index].racer[0]
    ) {
      winData.win = true;
      winData.amount +=
        racerSettings[playerData.bets[index].racer[0]].odds[betIndex] *
    ...
    if (
      gameData.revealResults[0] == playerData.bets[index].racer[0] &&
      gameData.revealResults[1] == playerData.bets[index].racer[1]
    ) {
      winData.win = true;
    ...
    if (
      ...
      if (
        gameData.revealResults[0] == playerData.bets[index].racer[1] ||
        gameData.revealResults[1] == playerData.bets[index].racer[1]
      ) {
        winData.win = true;
        winData.amount += betSettings[betIndex].payout;
    ...
    if (
      gameData.revealResults[0] == playerData.bets[index].racer[0] &&
      gameData.revealResults[1] == playerData.bets[index].racer[1] &&
      gameData.revealResults[2] == playerData.bets[index].racer[2]
    ) {
      winData.win = true;
      winData.amount += betSettings[betIndex].payout;
    }
    ...
    if (
      ...
      if (
        gameData.revealResults[0] == playerData.bets[index].racer[1] ||
        gameData.revealResults[1] == playerData.bets[index].racer[1] ||
        gameData.revealResults[2] == playerData.bets[index].racer[1]
      ) {
        if (
          gameData.revealResults[0] == playerData.bets[index].racer[2] ||
          gameData.revealResults[1] == playerData.bets[index].racer[2] ||
          gameData.revealResults[2] == playerData.bets[index].racer[2]
        ) {
          winData.win = true;
          winData.amount += betSettings[betIndex].payout;
    ...
    if (
      ...
      gameData.revealResults[1] == playerData.bets[index].racer[1] &&
      gameData.revealResults[2] == playerData.bets[index].racer[2] &&
      gameData.revealResults[3] == playerData.bets[index].racer[3]
    ) {
      winData.win = true;
      winData.amount += betSettings[betIndex].payout;
    }
    ...
    if (
      ...
      gameData.revealResults[1] == playerData.bets[index].racer[0] ||
      gameData.revealResults[2] == playerData.bets[index].racer[0] ||
      gameData.revealResults[3] == playerData.bets[index].racer[0]
    ) {
      if (
        gameData.revealResults[0] == playerData.bets[index].racer[1] ||
        gameData.revealResults[1] == playerData.bets[index].racer[1] ||
        gameData.revealResults[2] == playerData.bets[index].racer[1] ||
        gameData.revealResults[3] == playerData.bets[index].racer[1]
      ) {
        if (
          gameData.revealResults[0] == playerData.bets[index].racer[2] ||
          gameData.revealResults[1] == playerData.bets[index].racer[2] ||
          gameData.revealResults[2] == playerData.bets[index].racer[2] ||
          gameData.revealResults[3] == playerData.bets[index].racer[2]
        ) {
          if (
            gameData.revealResults[0] == playerData.bets[index].racer[3] ||
            gameData.revealResults[1] == playerData.bets[index].racer[3] ||
            gameData.revealResults[2] == playerData.bets[index].racer[3] ||
            gameData.revealResults[3] == playerData.bets[index].racer[3]
          ) {
            winData.win = true;
            winData.amount += betSettings[betIndex].payout;
...
function updateCredit() {
  var totalAmount = getCurrentBetAmount();
  creditTxt.text = creditRedTxt.text =
    textStrings.currency + addCommas(playerData.credit - totalAmount);
}
...
function createRacer(index) {
  var _difSpeed = randomIntFromInterval(8, 10) * 0.1;
  var _frameW = 131;
  var _frameH = 91;
...
function prepareRace() {
  ...
  if (billboardImg) {
    ...
    var _targetH =
      typeof canvasH !== 'undefined' && canvasH ? Math.round(canvasH * 0.12) : billboardImg.height;
    ...
    _m.a = _scale;
    _m.d = _scale;
    $.field['billboard'].graphics
      .beginBitmapFill(billboardImg, 'repeat', _m)
      .drawRect(0, 0, _drawW, _targetH);
  ...
  if (groundImg) {
    ...
    $.field['ground'].graphics
      .beginBitmapFill(groundImg, 'repeat')
      .drawRect(0, 0, _gDrawW, groundImg.height + 2);
  ...
  var containerItems = [
    ...
    $.field['ground'],
    $.field['endline'],
    racersShadowContainer,
    $.field['end'],
    racersContainer,
    dimShape,
  ];
  var validContainerItems = containerItems.filter(function (item) {
    return item != null;
  });
  ...
  if (gameData.isRestoring) {
    ...
    TweenMax.to(raceDisplayContainer, 0.5, {
      alpha: 1,
      overwrite: true,
      onComplete: function () {
        var delayNum = 1;
        for (var n = 0; n < racerSettings.length; n++) {
          ...
          TweenMax.to($.racer['raceicon' + n], 0.4, {
            delay: delayNum,
            x: oriX,
            alpha: 1,
            overwrite: true,
        ...
        for (var n = 0; n < racerSettings.length; n++) {
          ...
          TweenMax.to($.racer['racename' + n], 0.4, {
            delay: delayNum,
            x: oriX,
            alpha: 1,
            overwrite: true,
        ...
        TweenMax.to(raceDisplayMoveContainer, 0.5, {
          delay: 1,
          y: -200,
          ease: Expo.easeOut,
          overwrite: true,
        });
        TweenMax.to(raceDisplayContainer, 0.5, { delay: 4.5, alpha: 0, overwrite: true });
      },
    });

    scoreMoveContainer.alpha = 0;
    scoreMoveContainer.y = 50;
    TweenMax.to(scoreMoveContainer, 0.5, {
      delay: 4.5,
      overwrite: true,
      onComplete: function () {
        var delayNum = 0;
    ...
    TweenMax.to(gameRaceContainer, 8, {
      overwrite: true,
      onComplete: function () {
        playSound('soundGate');
        for (var n = 0; n < gameData.race.gatesOpen.length; n++) {
          var gateOpen = gameData.race.gatesOpen[n];
          TweenMax.to(gateOpen, 0.2, { scaleX: 1, overwrite: true });
        }

        TweenMax.to(gameRaceContainer, 0.1, {
          overwrite: true,
          onComplete: function () {
            gameData.race.start = true;
...
function endRace() {
  ...
  for (var n = 0; n < racerSettings.length; n++) {
    ...
    TweenMax.to($.racer['race' + n], tweenSpeed, {
      x: $.racer['race' + n].newX,
      y: $.racer['race' + n].newY,
      overwrite: true,
      onUpdate: updateRacerShadow,
      onUpdateParams: [n],
    });
  }

  var volumeData = { volume: 1 };
  TweenMax.to(volumeData, 2.5, {
    volume: 0,
    overwrite: true,
    onUpdate: function () {
  ...
  TweenMax.to(gameRaceContainer, 2.5, {
    overwrite: true,
    onComplete: function () {
  ...
  if (gameData.race.start) {
    ...
    for (var n = 0; n < racerSettings.length; n++) {
      if ($.racer['race' + n].delayNum <= 0) {
        ...
        $.racer['race' + n].newX =
          $.racer['race' + n].newX < gameData.race.rangeX[0]
            ? gameData.race.rangeX[0]
            : $.racer['race' + n].newX;
        $.racer['race' + n].newX =
          $.racer['race' + n].newX > gameData.race.rangeX[1]
            ? gameData.race.rangeX[1]
            : $.racer['race' + n].newX;
        $.racer['race' + n].newY =
          $.racer['race' + n].newY < gameData.race.rangeY[0]
            ? gameData.race.rangeY[0]
            : $.racer['race' + n].newY;
        $.racer['race' + n].newY =
          $.racer['race' + n].newY > gameData.race.rangeY[1]
            ? gameData.race.rangeY[1]
            : $.racer['race' + n].newY;
...
function updateRacerScore(raceProgress) {
  ...
  for (var n = 0; n < sortArr.length; n++) {
    ...
    scoreListContainer.setChildIndex(
      $.racer['icon' + racerIndex],
      scoreListContainer.numChildren - 1,
    );
    TweenMax.to($.racer['icon' + racerIndex], 0.2, { x: gameData.race.lists[n], overwrite: true });
  }
}
...
function animateBlink(obj, alpha) {
  var alphaNum = alpha == undefined ? 0.5 : alpha;
  obj.visible = true;
  obj.alpha = alphaNum;
  TweenMax.to(obj, 0.3, {
    alpha: 1,
      ...
      TweenMax.to(obj, 0.3, {
        ...
        onComplete: function () {
          TweenMax.to(obj, 0.3, {
            ...
            overwrite: true,
            onComplete: function () {
              TweenMax.to(obj, 0.3, { alpha: 0, overwrite: true });
            },
          });
        },
      });
  ...
  if (action == 'facebook') {
    if (shareSettings.customScore) {
      ...
      shareURL =
        ...
        encodeURIComponent(
          `${gameURL}share.php?title=${shareTitle}&url=${gameURL}&thumb=${gameURL}share.jpg`,
        );
    } else {
...
function restoreRace(progressPercent, elapsedMs, racersData, startTime, serverFinished) {
  ...
  if (
    typeof gameBetContainer === 'undefined' ||
    typeof gameRaceContainer === 'undefined' ||
    typeof scoreContainer === 'undefined'
  ) {
    console.log('Game containers not ready, waiting to restore race...');
    setTimeout(function () {
      restoreRace(progressPercent, elapsedMs, racersData, startTime, serverFinished);
    }, 200);
    return;
  }
  ...
  if (progressPercent > 0) {
    ...
    if (elapsedMs && gameSettings.raceSpeed) {
      ...
      if ($.field['ground'])
        $.field['ground'].x =
          ($.field['ground'].x - deltaS * gameSettings.raceSpeed) % $.field['ground'].tileW;
      if ($.field['billboard'])
        $.field['billboard'].x =
          ($.field['billboard'].x - deltaS * (gameSettings.raceSpeed * 0.8)) %
          $.field['billboard'].tileW;
      if ($.field['sky'])
        $.field['sky'].x =
          ($.field['sky'].x - deltaS * (gameSettings.raceSpeed * 0.3)) % $.field['sky'].tileW;
    ...
    if (racersData && Array.isArray(racersData) && racersData.length > 0) {
      var sortedRacers = racersData.slice(0).sort(function (a, b) {
        return b.x - a.x;
      });
